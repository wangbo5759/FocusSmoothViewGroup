...
import com.example.celllayout.AbsListView.OnScrollListener;
...
class FastScroller {
    ...
    private void useThumbDrawable(Context context, Drawable drawable) {
        ...
        if (drawable instanceof NinePatchDrawable) {
            mThumbW = ((int)TypedValue.applyDimension(1, 64.0F, context.getResources().getDisplayMetrics()));
            mThumbH = ((int)TypedValue.applyDimension(1, 52.0F, context.getResources().getDisplayMetrics()));
        } else {
    ...
    void scrollTo(float position) {
        ...
        if (sections != null && sections.length > 1) {
            ...
            } else*/ if (mList instanceof SmoothListView) {
                ((SmoothListView)mList).setSelectionFromTop(index + mListOffset, 0);
            } else {
        ...
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                if (mState > STATE_NONE && isPointInside(ev.getX(), ev.getY())) {
                    if (!ReflectUtil.invokeMethodForBoolean(View.class, "isInScrollingContainer", mList)) {
                        beginDrag();
                        return true;
                    }
