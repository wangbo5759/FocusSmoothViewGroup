 ...
 */
@RemoteView
public class SmoothListView extends AbsListView {
    /**
     ...
     * SohuTV: max scroll length change to the 90% of listview
     */
    private static final float MAX_SCROLL_FACTOR = 0.9f;
    ...
    /*
     * after the fling process, after the delay time, then set the state of idle.
     */
    private static final int FLING_END_DELAY = 400;
     ...
     * SohuTV: scroll animation duration
     */
    ...
    /*
     * SohuTV:the height of preview item-view (length to scroll more, to show the next item-view)  
     */
    ...
    /*
     * SohuTV:Not support the attribute which is defined in xml
     */
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (mHeaderViewInfos.size() > 0|| mFooterViewInfos.size() > 0) {
        	//SohuTV: not support the headerView
...
//    @android.view.RemotableViewMethod
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (!handled) {
            ...
            //SohuTV:up and down key-event handle it in list-view, otherwise return false to let parent to handle it 
    ...
    private boolean commonKey(int keyCode, int count, KeyEvent event) {
        ...
        //SohuTV: when the listview is scrolling , not responsing to key events
     ...
     */
    public boolean arrowScrollImpl(int direction) {
        if (getChildCount() <= 0) {
            return false;
        }
    ...
    /**
     * SohuTV:It will calculate the amount of item to scroll, every time you scroll to show bottom views
     * or top views
     */
    public int getVerticalFadingEdgeLength()
    {
      if ((this.mFadingEdgeLength == 0) && (getChildCount() > 1)){
	        View localView = getChildAt(0);
	        this.mFadingEdgeLength = localView.getHeight() / 2;
      }
      return this.mFadingEdgeLength ;
    }
    
    public void setVisibleViewExtraHeight(int visibleHeight){
    	mFadingEdgeLength = visibleHeight;
    }
     ...
     */
    protected int amountToScroll(int direction, int nextSelectedPosition) {
        ...
        if (direction == View.FOCUS_UP) {
            if (mTempRect.top < mListPadding.top) {
                ...
                if (positionOfNewFocus > 0) {
                	...
                	//If the scroll to the first child of list-view, scroll to the top of list-view,
                	//because the mTempRect.top is may not the top of the list-view
                	amountToScroll = mListPadding.top - getChildAt(positionOfNewFocus).getTop();
                }
            }
        } else {
            final int listBottom = getHeight() - mListPadding.bottom;
     ...
     * SohuTV:the scroll has animation.
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (mCachingActive && ReflectUtil.getBooleanField(View.class, "mCachingFailed")) {
     ...
     * SohuTV: no header or footer views
     */
   /* @Override
    ...
    /*View findViewByPredicateInHeadersOrFooters(ArrayList<FixedViewInfo> where,
        ...
        if (where != null) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (v != childToSkip && !ReflectUtil.view_isRootNamespace(v)) {
...
//            return getCheckedItemIds();
     ...
     * SohuTV:FlingRunnable notify the process of scrolling 
     */
    private class FlingRunnable implements Runnable {
        ...
        private final Interpolator mInterpolator = new AccelerateDecelerateInterpolator();/*new Interpolator() {
            public float getInterpolation(float t) {
            	//first acceleration, then deceleration
                return (float)(Math.cos(t * Math.PI + Math.PI)/2.0f + 0.5f);
            }
        };*/
        /**
        ...
        private void endFling(boolean scrollIntoSlots) {
            ...
            //detach or attach the view after scrolling process
            detachAndAttachScreenChildren(mDistance);
        }
    ...
    void trackMotionScroll(int deltaX) {
        ...
        ViewCompat.offsetChildrenTopAndBottom(this, deltaX);
        
        //to detach or attach the view after scrolling process
        //detachAndAttachScreenChildren(deltaX);
    ...
    private void detachAndAttachScreenChildren(int amount){
    	final int listBottom = getHeight() - mListPadding.bottom;
        final int listTop = mListPadding.top;
        final AbsListView.RecycleBin recycleBin = mRecycler;
        ...
        if (amount < 0) {
            ...
            int numChildren = getChildCount();
            View last = getChildAt(numChildren - 1);
            while (last != null && last.getBottom() < listBottom) {
                final int lastVisiblePosition = mFirstPosition + numChildren - 1;
                if (lastVisiblePosition < mItemCount - 1) {
                    last = addViewBelow(last, lastVisiblePosition);
                    numChildren++;
            ...
            View first = getChildAt(0);
            /*
             * notice that scroll up , not to remove a view below the bottom -listTop
             */
            ...
            // may need to pan views into top
            /*
             * notice that scroll down , to add a view on the top -listTop
             */
            while (first != null && (first.getTop() > -listTop) && (mFirstPosition > 0)) {
                first = addViewAbove(first, mFirstPosition);
                mFirstPosition--;
            }
